...
def load_oracle_prices(force=False, numoracleprices=500):
    cursor = None	
    chals = []
    if numoracleprices > 5000:
        raise ValueError(f"invalid number of oracle prices to load")
    while len(chals) < numoracleprices:
        path = f"oracle/prices"
        if cursor:
            path += f"?cursor={cursor}"
        result = api_call(path=path)
        print(f"-I- loaded {len(result['data'])} oracle prices")
        cursor = result.get('cursor')
...
def load_oracle_price_at_block(block,numoracleprices=2500):
    cursor = None	
    chals = []
    if numoracleprices > 5000:
        raise ValueError(f"invalid number of oracle prices to load")
    while len(chals) < numoracleprices:
        path = f"oracle/prices/{block}"
        if cursor:
            path += f"?cursor={cursor}"
        result = api_call(path=path)
        #print(f"-I- loaded {len(result['data'])} oracle prices")
...
def load_blocks(force=False, numoracleprices=25000):
    cursor = None	
    chals = []
    ...
    while len(chals) < numoracleprices:
        ...
        if not cursor:
            break
        #print(datetime.datetime.fromtimestamp(chals[0]['time']).strftime('%c'))
        #print(chals)
  
    for i in reversed(chals):
        as_of_time = datetime.fromtimestamp(i['time'])        
        #if (latest_date.date() != as_of_time.date()):
        if (as_of_time.date().day != latest_date):
            print(as_of_time)
            print(i['height'])
            latest_date = as_of_time.date().day
            oracle_price = load_oracle_price_at_block(i['height'])
            #print('return')
            #print(oracle_price)
            print(oracle_price['price'] / 100000000)
        #print(as_of_time)
        #latest_date = as_of_time.date
        #print(as_of_time.strftime('%c'))
    return chals
...
if __name__ == '__main__':
  ...
  #parser.add_argument('-x', help='report to run', choices=['poc_reliability', 'poc_v10', 'poc_summary'], required=True)    
    parser.add_argument('-x', choices=['refresh_hotspots','oracle_prices'], help="action to take")
